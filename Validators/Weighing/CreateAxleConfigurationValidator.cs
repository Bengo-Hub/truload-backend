...
/// <summary>
/// Validator for creating axle configurations
/// Enforces business rules: code uniqueness, axle count, GVW ranges, framework validation
/// </summary>
public class CreateAxleConfigurationValidator : AbstractValidator<CreateAxleConfigurationDto>
{
    public CreateAxleConfigurationValidator()
    {
        RuleFor(x => x.AxleCode)
            .NotEmpty().WithMessage("Axle code is required")
            .Length(1, 50).WithMessage("Axle code must be between 1 and 50 characters")
            .Matches(@"^[a-zA-Z0-9\*\|\-]+$").WithMessage("Axle code can only contain alphanumeric characters, asterisks (*), pipes (|), and hyphens (-)");
