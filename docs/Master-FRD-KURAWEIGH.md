...
| Actors | Field Officer (user) Truck driver **TruConnect middleware** (node/electron), HAENNI REST API, PAW Serial protocols, ANPR, Booms & camera systems (if integrated). |
| Pre-conditions (front end) | Officer logged in (or PWA has offline credentials cached). PWA service-worker registered and IndexedDB initialized (persistent storage requested). **TruConnect** running and connected to attached scale(s) (serial/ethernet/RF/HTTP). Scale calibration certificate valid and scale test completed for the day. Permissions: officer role authorizes weighing/prosecution actions. |
...
| Basic Flow (TruConnect) | Start middleware (electron) on client. Middleware connects to scales and exposes a **local Proxy API** or **WebSocket Server**. **Real-time Connection (Default)**: Streams live normalized JSON via WebSocket. **Polling Fallback**: If WebSocket or online connection is restricted, frontend polls the middleware directly or via backend proxy. **Supported Types**: 1. *Multideck (Static)*: ZM, Cardinal 1/2, 1310 via Serial/TCP/UDP. 2. *Mobile (Portable)*: PAW via RS-232, Haenni via HTTP REST API. Middleware normalizes all payloads regardless of scale type. On axle readings, middleware can post autoweigh events to configurable backend endpoints. |
